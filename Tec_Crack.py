...
if not user or not nonce or not md5Hash or not wordFile:  # makes sure all variables are filled
    usage()
...
realm = "Technicolor Gateway"  # These 3 variables don't appear to change
qop = "auth"
uri = "/login.lp"

HA2 = md5(
    "GET" + ":" + uri).hexdigest()  # This hash doesn't contain any changing variables, doesn't need to be recalculated

wordFile = open(wordFile, 'r')  # Opens the wordlist file
wordlist = wordFile.readlines()  # This enables us to use len()
length = len(wordlist)
...
for i in range(cores):  # Creates start and stopping points based on length of word list
    break_points.append(
        {"start": int(math.ceil((length + 0.0) / cores * i)), "stop": int(math.ceil((length + 0.0) / cores * (i + 1)))})
...
wordFile.close()  # Closes the wordlist file
screen_clear()
sys.exit(2)
